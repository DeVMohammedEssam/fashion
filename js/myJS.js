...
$(document).ready(function (){
   ...
   $("div:eq(0)").click(function(){
      'use-strict';
        xButton.parent().hide(100);
       $(".when-eye-clicked .close").parent().slideUp();
         /* hiding cover when clicking X*/
       $("div:eq(0)").fadeOut(200);      
   });
    ...
    $(theWholePicContainer).hover(
      function(){
          ...
          function imgShuffle(){
              var currentDiv="#"+divID;
               currentImg=$(currentDiv).children("div").children("img").first();
               if($(currentDiv).parent().attr("id")==="tshirts") currentImg.attr("src",objOfImgs.tshirts[i]);
              else if($(currentDiv).parent().attr("id")==="skirts") currentImg.attr("src",objOfImgs.skirts[i]);
              else if($(currentDiv).parent().attr("id")==="watches") currentImg.attr("src",objOfImgs.watches[i]);
              else if($(currentDiv).parent().attr("id")==="sandals") currentImg.attr("src",objOfImgs.sandals[i]);
              else if($(currentDiv).parent().attr("id")==="jewellery") currentImg.attr("src",objOfImgs.jewellery[i]);
              
              else if($(currentDiv).attr("id")==="15") currentImg.attr("src",objOfImgs.skirtsII[i]);
              else if($(currentDiv).attr("id")==="16") currentImg.attr("src",objOfImgs.jackets[i]);
              else if($(currentDiv).attr("id")==="17") currentImg.attr("src",objOfImgs.dresses[i]);
              else if($(currentDiv).attr("id")==="18") currentImg.attr("src",objOfImgs.jeans[i]);
          }
          
          /*repeating img shuffling*/
          setcontainer=setInterval(function(){imgShuffle();i++;if(i==3)i=0},500);
      ...
      function(){
        ...
        eyeIcon.animate({top:'-400px'}).slideUp();
          shadow.animate({left:'400px'},300).css({transform:'rotate(-80deg)'})
